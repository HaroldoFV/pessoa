...
namespace AspnetCoreReportDataExportSamples.Controllers
{
    public partial class OrdersController
    {
        ...
        {
            var result = DataLoader.Orders.Value.Where(x => x.Id == id);

            if (result.Count() == 0)
                return NotFound("Nenhuma informação encontrada.");
            ...
            return File(Encoding.UTF8.GetBytes(builder.ToString()), "application/vnd.ms-excel", "orders.csv");
        }
    }
}
